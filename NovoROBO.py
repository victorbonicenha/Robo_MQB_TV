...
def sleep_vigiando_welcome(page, segundos, limite_welcome=120):
    """
    Faz sleep de 'segundos'. A cada 15s checa se a welcome esta visivel.
    Se ficar mais de 'limite_welcome' segundos travada nela, levanta excecao.
    Se nao tiver welcome, eh um sleep normal.
    """
    ...
    while datetime.now().timestamp() < fim:
        try:
            visivel = page.locator(xpath_welcome).first.is_visible(timeout=1000)
        except:
            visivel = False
        ...
        if visivel:
            if welcome_desde is None:
                ...
                log("Welcome apareceu - iniciando contagem")
            else:
                parado = int(datetime.now().timestamp() - welcome_desde)
                log(f"Welcome travada ha {parado}s (limite {limite_welcome}s)")
                if parado >= limite_welcome:
                    telegram(f"Linha {cfg['linha']} travada na welcome ha {parado}s. Reiniciando.")
                    raise Exception(f"Travado na welcome por {parado}s")
        else:
...
def abrir_dashboard(page):
    ...
    iframe.locator("button:has(svg.animate-spin)").click(timeout=10000)
    sleep(espera_iframe)
    iframe.locator("button:has-text('Modo Tela Cheia')").click(timeout=10000)
    sleep(espera_iframe)
    iframe.locator("button:has(svg.lucide-x)").click(timeout=10000)
...
def tentar_abrir_dashboard_com_retry(page, tentativas=2, motivo=""):
    ...
    for tentativa in range(1, tentativas + 1):
        try:
            ...
            log("Linha aberta em tela cheia. Aguardando...")
            return True
        except Exception as e:
            ultimo_erro = e
...
def monitorar_dashboard(page):
    ...
    while True:
        log(f"Aguardando {tempo_att}s até próximo reload (vigiando welcome)...")
        sleep_vigiando_welcome(page, tempo_att, limite_welcome=120)
...
def run(playwright):
    while True:
        try:
            ...
            sleep(3)
            # Verifica se travou na tela #welcome apos o login
            log(f"Aguardando {tempo_att}s até próximo reload (vigiando welcome)...")
